...
const SessionsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            loginSession: _entity.loginSession,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info sessions updated successfully" });
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div>
                <p className="m-0">Login Session:</p>
                <InputText className="w-full mb-3" value={_entity?.loginSession} onChange={(e) => setValByKey("loginSession", e.target.value)}  />
            </div>
            <div>
...
export default connect(mapState, mapDispatch)(SessionsCreateDialogComponent);
