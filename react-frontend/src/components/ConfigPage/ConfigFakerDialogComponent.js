...
import configFakerFactory from "./configFakerFactory";
...
const ConfigFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = configFakerFactory(fakerCount,chataiidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("config").create(elem));
...
export default connect(mapState, mapDispatch)(ConfigFakerDialogComponent);
