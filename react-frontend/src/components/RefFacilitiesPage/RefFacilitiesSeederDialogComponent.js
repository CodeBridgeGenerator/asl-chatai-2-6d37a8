...
import seederData from "./refFacilities.seeder.json";
...
const RefFacilitiesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("refFacilities").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(RefFacilitiesSeederDialogComponent);
