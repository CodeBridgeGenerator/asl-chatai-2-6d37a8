...
import seederData from "./refFaDocs.seeder.json";
...
const RefFaDocsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("refFaDocs").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(RefFaDocsSeederDialogComponent);
