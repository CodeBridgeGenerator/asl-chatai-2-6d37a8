...
import refFaDocsFakerFactory from "./refFaDocsFakerFactory";
...
const RefFaDocsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [bankIdIds, setbankIdIds] = useState();
                

                const [facilityidIds, setfacilityidIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            .service("refBanks")
            .find({ query: { $select: ["_id","abbr"] } })
            .then((data) => {
              setbankIdIds(data.data);
            });
client
            .service("refFacilities")
            .find({ query: { $select: ["_id","type"] } })
            .then((data) => {
              setfacilityidIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = refFaDocsFakerFactory(fakerCount,bankIdIds,facilityidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refFaDocs").create(elem));
...
export default connect(mapState, mapDispatch)(RefFaDocsFakerDialogComponent);
