...
import seederData from "./samplePrompts.seeder.json";
...
const SamplePromptsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("samplePrompts").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(SamplePromptsSeederDialogComponent);
