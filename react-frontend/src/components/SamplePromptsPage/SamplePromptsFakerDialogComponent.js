...
import samplePromptsFakerFactory from "./samplePromptsFakerFactory";
...
const SamplePromptsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = samplePromptsFakerFactory(fakerCount,chataiidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("samplePrompts").create(elem));
...
export default connect(mapState, mapDispatch)(SamplePromptsFakerDialogComponent);
