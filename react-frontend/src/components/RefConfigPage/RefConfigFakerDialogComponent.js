...
import refConfigFakerFactory from "./refConfigFakerFactory";
...
const RefConfigFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = refConfigFakerFactory(fakerCount,chatAiIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refConfig").create(elem));
...
export default connect(mapState, mapDispatch)(RefConfigFakerDialogComponent);
