...
const UserPromptsSavedCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info userPromptsSaved updated successfully" });
    ...
    useEffect(() => {
                    ...
                    client
                        ...
                        .find({ query: { $limit: 10000 } })
                        .then((res) => {
                            setSaveduserid(res.data.map((e) => { return { name: e['name'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div>
                ...
                <Dropdown value={_entity?.saveduserid} optionLabel="name" optionValue="value" options={saveduseridOptions} onChange={(e) => setValByKey("saveduserid", e.value)} />
            </div>
            <div>
