...
const UserPromptsSavedDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.saveduserid?.name}</p>
    ...
    return (
        <DataTable value={items} onRowClick={onRowClick} scrollable rowHover paginator rows={10} rowClassName="cursor-pointer">
            <Column field="saveduserid" header="User" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
            <Column field="configid" header="Config" body={pTemplate2} style={{ minWidth: "8rem" }} />
            <Column field="prompt" header="Prompt" body={pTemplate3} style={{ minWidth: "8rem" }} />
            <Column field="others" header="Others" body={pTemplate4} style={{ minWidth: "8rem" }} />
