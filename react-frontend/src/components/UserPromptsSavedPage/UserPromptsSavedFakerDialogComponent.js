...
import userPromptsSavedFakerFactory from "./userPromptsSavedFakerFactory";
...
const UserPromptsSavedFakerDialogComponent = (props) => {
    ...
    const [loading, setLoading] = useState(false);
                ...
                const [saveduseridIds, setsaveduseridIds] = useState();
...
client
    ...
    const onRun = async () => {
        let fakeData = userPromptsSavedFakerFactory(fakerCount,chataiidIds,saveduseridIds,configidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userPromptsSaved").create(elem));
...
export default connect(mapState, mapDispatch)(UserPromptsSavedFakerDialogComponent);
