...
import seederData from "./userPromptsSaved.seeder.json";
...
const UserPromptsSavedSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("userPromptsSaved").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(UserPromptsSavedSeederDialogComponent);
