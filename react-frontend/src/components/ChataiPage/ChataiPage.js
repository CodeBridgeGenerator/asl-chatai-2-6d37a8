...
const ChataiPage = (props) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("chatai").remove(data[selectedEntityIndex]?._id);
            ...
            console.log({ error });
            props.alert({ title: "Chatai", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    return (
        <div className="mt-5">
            ...
            <ChataiEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ChataiCreateDialogComponent show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
